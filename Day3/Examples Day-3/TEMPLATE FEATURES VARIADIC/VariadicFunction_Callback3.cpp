...
template<typename RET_TYPE, typename FUNCTION_FORM, typename... PARAMETERS> enable_if_t<is_convertible_v<RET_TYPE, void>, void>
 U_Callback(FUNCTION_FORM x, PARAMETERS ...Args)
{
...
template<typename RET_TYPE, typename FUNCTION_FORM, typename... PARAMETERS> enable_if_t<!is_convertible_v<RET_TYPE, void>, RET_TYPE>
 U_Callback(FUNCTION_FORM x, PARAMETERS ...Args)
{
    cout <<"Common business started..." << endl;
    auto result = (*x)(Args...);
...
int main()
{
    U_Callback<void>(&fun1, 56.12f, 88.34);     // U_Callback(void(*x)(float,double),56.12f, 88.34);
    U_Callback<void>(&fun2, 200);               // U_Callback(void(*x)(int),200);
